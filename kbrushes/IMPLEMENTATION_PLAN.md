...
Run:
```bash
mkdir -p kbrushes/{adapters/tranzact,pipelines,schema,api/routes,dashboard}
touch kbrushes/adapters/tranzact/__init__.py
touch kbrushes/pipelines/__init__.py
touch kbrushes/api/__init__.py
touch kbrushes/api/routes/__init__.py
```
...
KBRUSHES_COMPANY_ID = os.environ["KBRUSHES_COMPANY_ID"]
...
**✅ Acceptance:** `python -c "import kbrushes.config"` runs without error (with a valid `.env`).
...
Write a one-off test script (not production code) at `kbrushes/scripts/test_api.py`:

```python
"""
Run once: python -m kbrushes.scripts.test_api
...
**File:** `kbrushes/adapters/tranzact/auth.py`
...
from kbrushes.config import TRANZACT_EMAIL, TRANZACT_PASSWORD, TRANZACT_BASE_URL
...
**File:** `kbrushes/adapters/tranzact/client.py`
...
import httpx
from kbrushes.config import TRANZACT_BASE_URL, REQUESTS_PER_MINUTE
from kbrushes.adapters.tranzact.auth import get_access_token
...
**File:** `kbrushes/adapters/tranzact/reports.py`
...
**File:** `kbrushes/schema/init.sql`

Run this on your Railway/Supabase Postgres instance. This is the **schema lock** — finalize it at end of Day 5 and treat it as immutable.
...
**File:** `kbrushes/pipelines/base.py`
...
import asyncpg
from kbrushes.adapters.tranzact.client import fetch_report
from kbrushes.config import DATABASE_URL
...
from datetime import date, timedelta
from kbrushes.pipelines.sales_invoices import SalesInvoicesPipeline
from kbrushes.pipelines.ar_aging import ArAgingPipeline
...
from kbrushes.pipelines.scheduler import scheduler
...
from kbrushes.api.routes import dashboard, ai_tool
...
import asyncpg
from kbrushes.config import DATABASE_URL
from kbrushes.schema import queries
...
from typing import Optional
from kbrushes.adapters.tranzact.reports import AI_WHITELIST
from kbrushes.adapters.tranzact.client import fetch_report
from kbrushes import schema
...
- [ ] Deploy to Railway — set all env vars in Railway dashboard, never in code
...
3. Add Python service → connect to Postgres
4. Set environment variables (from .env.example)
5. Build command: pip install -r requirements.txt
6. Start command: uvicorn kbrushes.api.main:app --host 0.0.0.0 --port $PORT
7. Run schema: railway run psql $DATABASE_URL < kbrushes/schema/init.sql
8. Run backfill: railway run python -m kbrushes.pipelines.backfill_all
9. Open /dashboard/index.html
```
...
| Railway single worker token cache leak | Low | Document multi-worker Redis upgrade path; don't premature-optimise |
