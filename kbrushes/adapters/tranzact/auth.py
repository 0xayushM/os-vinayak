...
def _do_login(base_url: str, email: str, password: str) -> None:
    ...
    if body.get("status") != 1:
        raise RuntimeError(f"TranzAct login failed: {body.get('message', body)}")
    ...
    _cache.set(
        ...
        refresh_expires_at=_decode_exp(refresh_token),
    )
    logger.info(
...
def _do_refresh(base_url: str) -> bool:
    ...
    try:
        ...
        response.raise_for_status()
        body = response.json()
        new_access = body.get("access") or body.get("data", {}).get("access_token")
        if not new_access:
            return False
        _cache.set(
